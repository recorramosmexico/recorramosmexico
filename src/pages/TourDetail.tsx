            ...
            <div className="sticky top-24">
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                  ...
                  {!user && (
                    <div className="bg-amber-50 text-amber-700 text-sm px-4 py-3 rounded-xl border border-amber-100 flex items-center gap-2">
                      <LogIn size={16} className="flex-shrink-0" />
                      <span>
                        {lang === 'en' ? 'You need to ' : 'Necesitas '}
                        <Link to={`/login?redirect=/tours/${slug}`} className="font-bold underline">
                          {lang === 'en' ? 'sign in' : 'iniciar sesión'}
                        </Link>
                        {lang === 'en' ? ' to proceed to payment.' : ' para proceder al pago.'}
                      </span>
                    </div>
                  )}
